...
function ShopV2_CommonButton_UIBP:Refresh(ProductID)
    ...
    if ProductData == nil then
        print("ShopV2_CommonButton_UIBP:Refresh ProductData is nil for " .. tostring(ProductID));
        return;
    end
    local CurrentPrice = UGCCommoditySystem.GetSellingPriceAfterDiscount(ProductID);
    print("[ShopDiag] CommonButton Refresh ProductID=" .. tostring(ProductID) .. " SellingPrice=" .. tostring(ProductData.SellingPrice) .. " CurrentPrice=" .. tostring(CurrentPrice))
