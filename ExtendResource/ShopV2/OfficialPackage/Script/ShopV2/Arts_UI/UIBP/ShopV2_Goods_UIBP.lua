...
function ShopV2_Goods_UIBP:OnUpdateItem(Item, Idx)
    ...
    if ProductData and ProductData.LimitType ~= ELimitType.NotLimited then
        self.LimitProducts[ProductID] = Item;
    end
end

function ShopV2_Goods_UIBP:SelectProduct(ProductID)
    print("[ShopDiag] Goods SelectProduct ProductID=" .. tostring(ProductID) .. " CurrentProducts has=" .. tostring(self.CurrentProducts[ProductID] ~= nil))
