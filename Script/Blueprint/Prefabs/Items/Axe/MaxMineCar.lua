...
function MaxMineCar:CanUseV2()
    ...
    return MaxMineCar.SuperClass.CanUseV2(self);
end

function MaxMineCar:OnUseV2()
    -- 点击只执行普通物品使用，变身由装备后的OnEquip触发
    ugcprint("[矿车物品] 点击使用")
    MaxMineCar.SuperClass.OnUseV2(self)
end

function MaxMineCar:OnDisuseV2()
    MaxMineCar.SuperClass.OnDisuseV2(self);
    ...
    if Player and UGCPersistEffectSystem then
        local FullPath = UGCGameSystem.GetUGCResourcesFullPath("Asset/Blueprint/Prefabs/Skills/MaxVehicle.MaxVehicle_C")
            ...
            if ExistSkills and #ExistSkills > 0 then
                for _, SkillInst in ipairs(ExistSkills) do
                    if UE.IsValid(SkillInst) then
                        ...
                        ugcprint("[矿车物品] ✅ 已移除MaxVehicle技能实例")
                    end
                end
            end
        end
...
function MaxMineCar:CanEquip()
    local Player = GetPlayerPawnFromItem(self)
    if IsVehicleBroken(Player) then
        StopMineCarVisual(Player)
        NotifyRepairRequired(Player)
        ugcprint("[VehicleRepair] Advanced mining truck CanEquip blocked by repair state")
        return false
    end
    if MaxMineCar.SuperClass.CanEquip then
        return MaxMineCar.SuperClass.CanEquip(self)
    end
    return true
end

function MaxMineCar:OnEquip()
    MaxMineCar.SuperClass.OnEquip(self);
...
function MaxMineCar:OnUnEquip()
    MaxMineCar.SuperClass.OnUnEquip(self);
