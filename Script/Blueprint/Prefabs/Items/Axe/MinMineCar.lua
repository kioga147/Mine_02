...
function MinMineCar:CanUseV2()
    ...
    return MinMineCar.SuperClass.CanUseV2(self);
end

function MinMineCar:OnUseV2()
    -- 点击只执行普通物品使用，变身由装备后的OnEquip触发
    ugcprint("[矿车物品] 点击使用")
    MinMineCar.SuperClass.OnUseV2(self)
end

function MinMineCar:OnDisuseV2()
    MinMineCar.SuperClass.OnDisuseV2(self);

    local Player = GetPlayerPawnFromItem(self)
    ugcprint("[矿车物品] 取消使用矿车物品，玩家:", tostring(Player))

    -- 取消使用（松开攻击键）不再取消矿车模式
    -- 矿车模式由装备/卸载控制，这里只清理技能
...
function MinMineCar:CanEquip()
    ...
    if MinMineCar.SuperClass.CanEquip then
        return MinMineCar.SuperClass.CanEquip(self)
    end
    return true
end

function MinMineCar:OnEquip()
    MinMineCar.SuperClass.OnEquip(self);
...
function MinMineCar:OnUnEquip()
    MinMineCar.SuperClass.OnUnEquip(self);
