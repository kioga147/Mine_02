---@class Bauxite_C:BP_UGC_DamagableActor_C
...
function Bauxite:ReceiveBeginPlay()
    self.ShakeTime = 0
    self.CacheZ = self.StaticMesh:GetRelativeTransform().Translation.Z
    if MineZoneManager and UGCGameSystem.IsServer() then
        MineZoneManager.RegisterOre(self, "Bauxite")
    end
end

function Bauxite:PreOverrideDamage(Damage, EventInstigator, DamageCauser, DamageContext)
...
function Bauxite:PostTakeDamageEvent(Damage, EventInstigator, DamageCauser, DamageContext)
...
function Bauxite:GetReplicatedProperties()
    return 'ShakeSpeed', 'ShakeAmplitude'
end

function Bauxite:ReceiveTick(DeltaTime)
...
function Bauxite:BPDie(KillingDamage, EventInstigator,DamageCauser,DamageEvent,DamageTypeID)
