---@class Coal_C:BP_UGC_DamagableActor_C
...
function Coal:PreTakeDamageEvent(Damage, EventInstigator, DamageCauser, DamageContext)
    local mineLevel = UGCAttributeSystem.GetGameAttributeValue(self, "MineLevel")
    ugcprint("[矿石挖掘] 煤矿等级:", mineLevel)
...
---@param DamageCauser AActor 伤害来源
---@param DamageContext FGameMagnitudeContext  伤害上下文
function Coal:PostTakeDamageEvent(Damage, EventInstigator, DamageCauser, DamageContext)
...
function Coal:GetReplicatedProperties()
    return 'ShakeSpeed', 'ShakeAmplitude'
end

function Coal:ReceiveTick(DeltaTime)
