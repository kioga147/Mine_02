---@class Iron_ore_C:BP_UGC_DamagableActor_C
...
function Iron_ore:ReceiveBeginPlay()
    self.ShakeTime = 0
    self.CacheZ = self.StaticMesh:GetRelativeTransform().Translation.Z
    if MineZoneManager and UGCGameSystem.IsServer() then
        MineZoneManager.RegisterOre(self, "IronOre")
    end
end

function Iron_ore:PreOverrideDamage(Damage, EventInstigator, DamageCauser, DamageContext)
...
function Iron_ore:PostTakeDamageEvent(Damage, EventInstigator, DamageCauser, DamageContext)
...
function Iron_ore:GetReplicatedProperties()
    return 'ShakeSpeed', 'ShakeAmplitude'
end

function Iron_ore:ReceiveTick(DeltaTime)
...
function Iron_ore:BPDie(KillingDamage, EventInstigator,DamageCauser,DamageEvent,DamageTypeID)
