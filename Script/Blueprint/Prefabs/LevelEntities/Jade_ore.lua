---@class Jade_ore_C:BP_UGC_DamagableActor_C
...
function Jade_ore:ReceiveBeginPlay()
    self.ShakeTime = 0
    self.CacheZ = self.StaticMesh:GetRelativeTransform().Translation.Z
    if MineZoneManager and UGCGameSystem.IsServer() then
        MineZoneManager.RegisterOre(self, "JadeOre")
    end
end

function Jade_ore:PreOverrideDamage(Damage, EventInstigator, DamageCauser, DamageContext)
...
function Jade_ore:PostTakeDamageEvent(Damage, EventInstigator, DamageCauser, DamageContext)
...
function Jade_ore:GetReplicatedProperties()
    return 'ShakeSpeed', 'ShakeAmplitude'
end

function Jade_ore:ReceiveTick(DeltaTime)
...
function Jade_ore:BPDie(KillingDamage, EventInstigator,DamageCauser,DamageEvent,DamageTypeID)
