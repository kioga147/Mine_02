---@class Quartz_C:BP_UGC_DamagableActor_C
...
function Quartz:ReceiveBeginPlay()
    self.ShakeTime = 0
    self.CacheZ = self.StaticMesh:GetRelativeTransform().Translation.Z
    if MineZoneManager and UGCGameSystem.IsServer() then
        MineZoneManager.RegisterOre(self, "Quartz")
    end
end

function Quartz:PreOverrideDamage(Damage, EventInstigator, DamageCauser, DamageContext)
...
function Quartz:PostTakeDamageEvent(Damage, EventInstigator, DamageCauser, DamageContext)
...
function Quartz:GetReplicatedProperties()
    return 'ShakeSpeed', 'ShakeAmplitude'
end

function Quartz:ReceiveTick(DeltaTime)
...
function Quartz:BPDie(KillingDamage, EventInstigator,DamageCauser,DamageEvent,DamageTypeID)
